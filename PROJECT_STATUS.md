...
🟡 ADR-007 — Implementação, Sprint A1 (identidade por linha e schema estrutural do CSV): CONCLUÍDA — `adapters.storage.salvar_lote` passa a exigir exatamente os cabeçalhos `nome`, `whatsapp`, `produto` (ordem livre, sem duplicatas, sem colunas extras), mapeados por nome de coluna, nunca por posição; apenas `.csv` é aceito (Excel removido); cada linha recebe `registro_coleta_id` (UUID4) próprio.
🟡 ADR-007 — Implementação, Sprint A2 (validação de conteúdo, normalização de WhatsApp e separação válidos/inválidos): CONCLUÍDA — cada linha é validada isoladamente (nome/produto vazios ou só com espaços são ausentes; textos literais "nan"/"none"/"null" são preservados como conteúdo real); WhatsApp normalizado decidindo o código do país pelo comprimento do número limpo, nunca por `startswith("55")` isoladamente (o DDD 55, Santa Maria/RS, tornaria essa checagem ambígua), e validado contra os 67 Códigos Nacionais oficiais da Anatel; registros inválidos são preservados em `clientes_invalidos` com `registro_coleta_id`, valores brutos exatos e motivo(s) objetivo(s), sem bloquear os válidos; contagens `total_recebido`/`total_valido`/`total_invalido` registradas (`total_clientes` mantido como campo legado, equivalente a `total_valido`); lote sem nenhum registro válido é criado e preservado normalmente, nunca tratado como erro de upload, com status `"sem_registros_validos"`; `core/scheduler.py` bloqueia esse lote em dois níveis — exclusão em `verificar_lotes_prontos()` e recusa determinística e sem escrita em `disparar_lote()` — com compatibilidade preservada para lotes anteriores à Sprint A2 (sem `total_valido`, via fallback para `total_clientes`).
🟡 ADR-007 — Implementação, Sprint A3 (correção append-only de registros inválidos): CONCLUÍDA — funções internas apenas, **sem nenhuma rota HTTP** (`app.py` intocado; rotas, autenticação e MFA do Operador Interno pertencem à Sprint D): parser estrutural independente do CSV de correção (`codigo_tecnico,nome,whatsapp,produto`, ordem livre, sem duplicatas/extras, CSV apenas com cabeçalho recusado como `csv_correcao_sem_registros`, sem criar histórico/auditoria); `codigo_tecnico` validado sintaticamente como UUID4 antes de qualquer busca (ausente/malformado nunca dispara busca local ou global); resolução restrita ao lote/Operação informado — busca em outros lotes é somente diagnóstica, nunca usada para aceitar; histórico append-only sem campo mutável de "corrente"/"substituída" (versão corrente sempre a de maior `numero_versao`); inicialização tardia e honesta do histórico para registros da Sprint A2, distinguindo `recebido_em` (M0 original) de `representacao_historica_criada_em` (agora) e marcando `recebido_por: nao_registrado_legado` em vez de atribuir retroativamente uma identidade inexistente; fronteira M0+192h testada deterministicamente (relógio injetável via parâmetro opcional `agora` em `calcular_d8`/`processar_uma_correcao`, sem alterar a regra existente) — correção exatamente na fronteira é tardia; relatório de correção gerado somente em memória via `csv.writer`; recusas (tardia, código inexistente, outra Operação, já válido, código ausente/inválido) auditadas em arquivos individuais provisórios (`data/correcoes_rejeitadas/`, escrita atômica), nunca alterando o lote; corrigido um deadlock real (`threading.Lock` não reentrante sendo readquirido por `salvar_lote_atomico` dentro de uma seção já protegida), com teste de deadlock e de concorrência real (duas correções simultâneas ao mesmo registro, sem perda de atualização, sequência de versões sem lacunas). Persistência definitiva (PostgreSQL) permanece para a Sprint B. **Nenhum envio operacional foi liberado nesta sprint.**

🟢 ADR-008 — Persistência Operacional Imutável, Claims e Idempotência: APROVADA E CONGELADA — decide PostgreSQL como persistência definitiva exclusivamente para o domínio operacional da ADR-007, sem extensão automática a nenhum outro domínio (inclusive a Trajetória Contínua já decidida pela ADR-006, cujo escopo permanece distinto); define evento imutável e projeção corrente como conceitos gerais; claim ("Reserva de Processamento") individual por `registro_coleta_id`, com token de posse; tempos operacionais aprovados (validade inicial de 5 minutos, heartbeat a cada 60 segundos, renovação por mais 5 minutos), não ajustáveis por configuração informal; três estados de projeção (`ativo`, `liberado`, `expirado_pendente_revisao`), distintos de um catálogo fechado de seis eventos imutáveis do ciclo de vida de claims (`claim_criado`, `heartbeat_registrado`, `claim_liberado`, `claim_expirado`, `revisao_de_abandono_registrada`, `claim_reatribuido`); proibição de reatribuição automática, com revisão explícita obrigatória; neutralidade absoluta de claim/heartbeat em relação a disparo e a `numero_tentativa`; exigência de transação e idempotência persistente entre processos e workers; migração por corte controlado (pausa de escritas, backup, importação, validação, mudança de fonte de verdade), sem escrita dupla permanente; preservação dos JSONs legados congelados, com manifesto e checksums, sem prazo de exclusão presumido por esta ADR. Nenhuma alteração à ADR-007, que permanece intocada e congelada.
...
🟢 ADR-007 — Sprint B, Subetapa B2 (Infraestrutura, Conexão e Migrations): CONCLUÍDA — PostgreSQL 17 local, de uso exclusivo de desenvolvimento/teste, com `psycopg` v3 síncrono e Alembic standalone (o SQLAlchemy trazido transitivamente pelo Alembic é uso interno da própria ferramenta de migration, sem adoção do ORM pela aplicação). Provisionados os bancos `nsi_dev` e `nsi_test`, as roles de migração `nsi_dev_migrator`/`nsi_test_migrator` (distintas das quatro roles funcionais já aprovadas para a Sprint B3 — `nsi_eventos_owner`, `nsi_aplicacao`, `nsi_expiracao`, `nsi_operador_restrito`) e o schema `nsi_operacional` nos dois bancos. Aplicada a migration bootstrap vazia (`0001`) em ambos, criando exclusivamente `nsi_operacional.alembic_version` — **nenhuma tabela de negócio foi criada nesta subetapa**. Suíte de testes: **246/246 aprovados**, incluindo os 3 testes de integração PostgreSQL real (antes pulados por ausência de banco; agora executados de fato contra `nsi_test`, com zero pulados). Dois problemas reais foram encontrados e corrigidos durante a validação, em `migrations/env.py`: (a) seleção explícita do dialeto `postgresql+psycopg` (o SQLAlchemy assumia `psycopg2` por padrão, não instalado neste projeto); (b) encerramento da transação de leitura ("autobegin" do SQLAlchemy 2.0) antes de iniciar a transação própria do Alembic — sem isso, `alembic upgrade` reportava sucesso sem persistir nada. **As subetapas B3.3/B3.4 ainda não foram iniciadas.**

🟢 ADR-007 — Sprint B, Subetapa B3.1 (Provisionamento administrativo, roles e isolamento): CONCLUÍDA — a Sprint B3 foi dividida em quatro subetapas fixas (B3.1 provisionamento; B3.2 migration `0002` — tabelas; B3.3 migration `0003` — as seis funções `SECURITY DEFINER`; B3.4 testes finais e documentação), decisão registrada em `docs/implementation/SPRINT-B-ESPECIFICACAO-TECNICA.md` (Seção 18). Provisionamento administrativo idempotente e seguro, em três fases (`scripts/postgres_local/provisionar_b3_roles.sql`): preflight completo, somente leitura, validando antecipadamente as quatro roles, as duas memberships dos migrators e a propriedade do schema nos dois bancos — qualquer estado inesperado aborta sem escrita e sem correção silenciosa; convergência executando apenas o que o preflight já autorizou; pós-validação completa reconfirmando tudo a partir do zero. Executado manualmente duas vezes contra o PostgreSQL 17 local — ambas concluindo com "Fase 3 (pós-validação completa) passou integralmente", comprovando a convergência idempotente sem nenhuma alteração destrutiva na segunda execução. Quatro roles funcionais criadas com atributos de segurança explícitos (`NOLOGIN`/`LOGIN` + `NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS`): `nsi_eventos_owner` (dona do schema, sem login), `nsi_aplicacao`, `nsi_expiracao`, `nsi_operador_restrito`. Isolamento de permissões: `PUBLIC` sem `CONNECT` em `nsi_dev`/`nsi_test`; cada migrator com `CONNECT` explícito restrito ao próprio banco; as três roles funcionais com `CONNECT` nos dois bancos locais; `USAGE` no schema `nsi_operacional` concedido aos migrators e às três roles funcionais; propriedade do schema transferida para `nsi_eventos_owner` nos dois bancos, sem afetar `alembic_version` (permanece com o migrator correspondente). `config.py` estendido com `resolver_url_banco_papel()`, função única e tipada para as seis variáveis funcionais (`DATABASE_URL_NSI_*`/`TEST_DATABASE_URL_NSI_*`), reaproveitando as proteções já validadas em B2.1 (papel e ambiente fechados, sem fallback, banco e usuário exigidos exatos, validação de `sslmode`, nenhuma credencial exposta em exceção/log/saída). Execução real: senhas das três roles `LOGIN` definidas interativamente via `\password`; as seis URLs configuradas localmente no `.env`, fora do versionamento. Reversão administrativa (`scripts/postgres_local/desprovisionar_b3_roles.sql`) implementada e documentada — exige revisão exatamente `0001` nos dois bancos, verifica ausência de objetos residuais antes de qualquer `DROP ROLE`, preserva o `CONNECT` dos migrators, nunca restaura `CONNECT` a `PUBLIC` — mas **não executada** nesta subetapa; cenários destrutivos permanecem exclusivamente manuais, documentados em `docs/implementation/PROCEDIMENTO-MANUAL-B3-DESTRUTIVO.md`, nunca contra `nsi_dev`/`nsi_test`. Testes de integração real, exclusivamente leitura (nunca `CREATE`/`ALTER`/`GRANT`/`REVOKE`/`DROP`): **20/20 aprovados**. Suíte completa do projeto: **331/331 aprovados**, nenhuma falha, nenhum pulado. **Nenhuma tabela de negócio, evento, claim ou função `SECURITY DEFINER` foi criada nesta subetapa.**

🟢 ADR-007 — Sprint B, Subetapa B3.2 (Migration `0002` — tabelas, constraints, índices e trigger defensiva): CONCLUÍDA — objetos criados por `migrations/versions/0002_claims_eventos_comandos_idempotentes.py`, executando integralmente como `nsi_eventos_owner`: `nsi_operacional.claims`, `nsi_operacional.eventos_claim`, `nsi_operacional.comandos_idempotentes`, com todas as `CHECK`s, a `PRIMARY KEY` composta e as `UNIQUE` normais já aprovadas na Parte 1 da B3; a FK composta diferível `claims_revisao_atual_fk` (`DEFERRABLE INITIALLY DEFERRED`); os índices únicos parciais `eventos_claim_expirado_unico` e `eventos_claim_origem_claim_id_unica`; a função `fn_bloquear_alteracao_eventos_claim` e a trigger `eventos_claim_bloqueia_alteracao` (`BEFORE UPDATE OR DELETE`, bloqueia inclusive o owner); `REVOKE ALL` explícito de `PUBLIC` e das três roles funcionais em cada tabela e na função da trigger — nenhuma role funcional com DML direto. Três defeitos encontrados e corrigidos **exclusivamente nos testes, nunca na migration**: suposição de estado inicial incompatível com o fluxo real de execução; dado de teste incoerente (`concluido_em` anterior a `criado_em`) num caso de `comandos_idempotentes`; `SET CONSTRAINTS` sem qualificação de schema, falhando por resolução via `search_path` do migrator (que não inclui `nsi_operacional`). Um quarto ponto, também exclusivo dos testes, corrigido na validação real: o inventário estrutural usava `information_schema.tables` (filtrada por privilégio — ocultava as três tabelas porque o migrator não tem DML direto nelas, membership `WITH INHERIT FALSE` de B3.1), substituída por `pg_catalog.pg_tables` (sem esse filtro). Testes endurecidos para nunca expor DSN em caso de falha (URL obtida internamente via `request.getfixturevalue`, nunca como parâmetro nomeado; nenhuma asserção compara a DSN bruta contra saída capturada). Execução real: `0002` aplicada primeiro em `nsi_test` (suíte específica da B3.2: **97/97 aprovados**); ciclo real comprovado `0002 → 0001 → 0002`, com o gate de identidade (`nsi_test`, servidor local, porta 5432, `nsi_test_migrator`) confirmado antes do único `downgrade`, terminando em `0002`; identidade de `nsi_dev` confirmada antes de qualquer ação, `0002` aplicada em `nsi_dev` **somente por `upgrade`** — nenhum `downgrade` executado em `nsi_dev` em nenhum momento desta subetapa; `nsi_dev` termina em `0002`. Suíte completa do projeto, com integração real obrigatória: **428/428 aprovados, nenhuma falha, nenhum pulado**. **Nenhuma das seis funções `SECURITY DEFINER` nem a migration `0003` foram criadas nesta subetapa — pertencem integralmente à Subetapa B3.3, ainda não iniciada.**
...
A fundação arquitetural da chave de idempotência persistente entre processos/workers e do schema conceitual de persistência foi decidida pela ADR-008 (APROVADA E CONGELADA); a implementação concreta permanece pendente, sequenciada pelo `ROADMAP-SPRINTS-B-G.md` (Sprint B em planejamento). As demais pendências desta seção seguem inalteradas.
